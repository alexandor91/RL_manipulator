...
#define GRIP_BASE  "gripperbase"
#define GRIP_LEFT  "gripper_left"
...
#define COLLISION_POINTMID  "arm::gripper_middle::middle_collision"
#define COLLISION_FILTER2  "arm::link2::collision2"
// Animation Steps
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
    ...
    const float DistanceToGoal = BoxDistance(propBBox, gripBBox); 
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		/
		*/
                //strcmp(strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0) /* for any arm part coming into contact with tube*/
	
		if ((strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0) && (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_FILTER2) != 0))
		{
			rewardHistory = 20.0f;
			newReward  = true;
			endEpisode = true;
			return;
                ...
                else if ((strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0) && (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_FILTER2) == 0))
		        rewardHistory = -5.0f - 10.f * DistanceToGoal;
			newReward  = true;
			endEpisode = true;
			return;
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	// get the bounding box for the gripper
	physics::LinkPtr gripper  = model->GetLink(GRIP_NAME);
	physics::LinkPtr gripper_base  = model->GetLink(GRIP_BASE);
		
	const math::Box& gripBBox = gripper->GetBoundingBox();
	const math::Box& gripBoxBase = gripper_base->GetBoundingBox();
	...
	if( hadNewState && !newReward )
	{
		...
		*/ 	
		...
		{
                 const float distGoaltmp = BoxDistance(propBBox, gripBBox);
			...
			if( episodeFrames > 1 )
			{
			 ...
			 const float distDelta  = lastGoalDistance - distGoaltmp;
                         std::cout<<"distDelta:  "<<distDelta<<std::endl;
                         if (distDelta > 0)
                            {
                            rewardHistory = 10 * exp(-distGoaltmp);
                 	        newReward     = true;
                 	    //endEpisode    = false;
		       }
		       else if (distDelta <= 0)
		       {
		         rewardHistory = -2.0f - distGoaltmp;  // * exp(-distGoaltmp);
		         newReward     = true;
		         //endEpisode    = false;
		       }
		      /*else
		      ...
		      Eldestreward = Lastreward;
		      Lastreward = rewardHistory;
		      /*if ((Eldestreward + Lastreward + rewardHistory) < -2 )
		      {  rewardHistory = -4.0f - distGoaltmp;
		         newReward     = true;
                         endEpisode = false;
                      }*/
				// compute the smoothed moving average of the delta of the  distance to the goal
				/*avgGoalDelta  =  alpha * avgGoalDelta + (1 - alpha) * distDelta;//alpha * (10.f * exp(-distGoaltmp)) + (1 - alpha)*distDelta;//alpha * avgGoalDelta + 1e4 * (1 - alpha) * distDelta;
                std::cout<<"avgDelta:  "<<avgGoalDelta<<std::endl;
				rewardHistory = avgGoalDelta;
				newReward     = true;	*/
			}
               lastGoalDistance = distGoaltmp;
		}
	}
